 ...
 * gallery card anatomy: a token frame (thin or thick border, radius/400,
 * clipped corners), a bottom title — over the dark scrim for gallery tiles,
 * bare for covers — and an optional top-right `action` slot.
 *
...
export type MediaCardFrame = 'thin' | 'thick'
...
export type MediaCardProps = Omit<ComponentPropsWithRef<'div'>, 'title'> & {
  ...
  scrim?: boolean
  /** Border weight: `thick` gallery tile (default) or `thin` cover. */
...
function Root({
  ...
  return useRender({
    ...
    props: {
      className: cx('q-media-card', FRAME_CLASS[frame], className),
      children: content,
      ...props,
    },
  })
