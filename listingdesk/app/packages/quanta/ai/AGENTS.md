...
`Grid`: `cols` (`number | 'auto-fit' | 'auto-fill'`; default `1`), `minColWidth` (for auto-fit/fill), `gap`/`gapX`/`gapY` (`1..24` scale), `flow` (`'row' | 'col' | 'dense'`), `align`/`justify`, `animate` (FLIP). `Grid.Item`: `colSpan`/`rowSpan`/`colStart` (`1..12`), `flipKey`. `VirtualGrid<Item>`: `items` (required), `renderItem` (required), `rowHeight` (px, required), `cols` or `minColWidth`, `gap`, `overscan` (default `3`), `getKey`, `height` (default `'32rem'`). Pair with `Media.Video autoPlayInView`.
